...
class carreras_view():
    def __init__(self, root) -> None:
        ...
        self.ventana_modal.title("Gestion de Carreras")
        ...
        # self.style.configure("Treeview.Heading", background='grey', foreground='white', relief=tk.SUNKEN)

        #Define el Nombre y el ancho en píxeles de las columnas.
        columns = {'Id': 50, 'Nombre': 300, 'Inicio': 80, 'Fin': 80, 'Duración': 80}
        ...
        statusbar = tk.Label(self.frame4, text='Tabla Carreras', bd=1, relief=tk.SUNKEN, anchor=tk.W)
    ...
    def getCursorInicio(self) -> str:
        selectedLbData = self.treeview.selection()[0]
        inicio = self.treeview.item(selectedLbData)['values'][2]
        return inicio

    def getCursorFin(self):
        selectedLbData = self.treeview.selection()[0]
...
class modalWindow:
    def __init__(self, parent, Nombre, datos=()) -> None:
        ...
        self.modal.geometry(centrar(alto=280, ancho=410, app=self.parent))
        ...
        self.textvarInicio = tk.StringVar()
        self.entryInicio = ttk.Entry(self.frame1, textvariable=self.textvarInicio, width=50)
        ...
        self.entryFin = ttk.Entry(self.frame1, textvariable=self.textvarFin, width=50)
    ...
    def getInicio(self):
        return datetime.strptime(self.textvarInicio.get(), "%Y-%m-%d")
        # '%Y-%m-%d'

    def setInicio(self, inicio: str) -> None:
        self.textvarInicio.set(inicio)

    def getFin(self):
        return datetime.strptime(self.textvarFin.get(), "%Y-%m-%d")
        # '%Y-%m-%d'

    def setFin(self, Fin: str) -> None:
        self.textvarFin.set(Fin)
